...
2. **Source → public flow.** Raw development happens in a private source repo. This repo receives only protected output. Never weaken protection layers (`verify()`, `mustContain`, omni-aware markers).
...
10. **No AI notes in shipped code.** `omni/voyager`, `omni/omni` and plugin files must contain
   ...
   only in `AI/` files. (In the private source repo, AI annotation is fully allowed.)
...
14. **Two-repo sync — only when needed.** Mirror to the other repo ONLY if the change affects the
    ...
    public docs stay here; raw sources, dist output and private notes stay there.
