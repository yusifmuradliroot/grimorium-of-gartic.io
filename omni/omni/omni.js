...
(function () {
    ...
    function executePlugin(code, src, plugin) {
        if (/\.fs([?#]|$)/.test(src)) return executePacked(code, plugin.id);
    ...
    function executePacked(code, pluginId) {
        // .fs payloads: tag + signature verified inside the embedded runner.
        // Runner refusals (null) and exceptions both mean failure.
        try {
            if (typeof ForgeScript === 'undefined' || !ForgeScript) return false;
            const r = ForgeScript.run(code);
            if (r === null) return false;
            vmInstances[pluginId] = { packed: true, tracked: new Set() };
            return true;
        } catch (e) {
            return false;
        }
