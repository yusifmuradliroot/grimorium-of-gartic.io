// ==UserScript==
// @name         Omni
// @namespace    omni-loader
// @version      2.3
// @description  Omni loader — self-checks version, then runs the framework (.fs via embedded runner).
// @match        https://gartic.io/*
// @grant        GM_xmlhttpRequest
// @grant        GM.xmlHttpRequest
// @grant        GM_info
// @grant        unsafeWindow
// @run-at       document-start
// ==/UserScript==
var __f=function(s){var o='',i=0;for(;i<s.length;i+=2){o+=String.fromCharCode(parseInt(s.substr(i,2),16)^0x5A);}return o;};
(function () {
    'use strict';
    const av = typeof unsafeWindow !== 'undefined' ? unsafeWindow : window;
    if (av.__omniVoyager) return;
    av.__omniVoyager = true;

    
    var ForgeScript={version:4,h:function(s){var h=0x811c9dc5,ah=0;for(;ah<s.length;ah++){h^=s.charCodeAt(ah);h=Math.imul(h,0x01000193)>>>0;}return ("0000000"+h.toString(16)).slice(-8);},b:function(t){var l=t.indexOf("\n");if(l<0||t.slice(0,l)!=="FS:2")return null;var ar=t.slice(l+1),ak=ar.indexOf("\n");if(ak<0)return null;var o=null;try{o=JSON.parse(ar.slice(0,ak));}catch(e){return null;}var d=ar.slice(ak+1).split("\n").filter(function(x){return x.length;});if(!o||!o.o||!o.s||o.o.length!==d.length||o.o.length<1)return null;var c=[],ah,j,q=[],t=0;for(ah=0;ah<o.o.length;ah++){if(o.o[ah]<0||o.o[ah]>=d.length)return null;c.push(d[o.o[ah]]);var s=atob(d[o.o[ah]]),k=(90^((ah*31+7)%256)),u=new Uint8Array(s.length);for(j=0;j<s.length;j++)u[j]=s.charCodeAt(j)^k;q.push(u);t+=u.length;}if(this.h("FS:2\n"+o.o.join(",")+"\n"+c.join(""))!==o.s)return null;var a=new Uint8Array(t),p=0;for(ah=0;ah<q.length;ah++){a.set(q[ah],p);p+=q[ah].length;}try{return new TextDecoder().decode(a);}catch(x){return null;}},run:function(t){var l=t.indexOf("\n");if(l<0||t.slice(0,l)!=="FS:2")return null;var a=Date.now();debugger;if(Date.now()-a>100)return null;var c=null;try{c=this.b(t);if(c==null)return null;return Function(c)();}finally{c="";t="";}}};


    const z = __f("322e2e2a29607575283b2d743d332e322f382f293f283935342e3f342e7439353775232f29333c372f283b3e36332835352e753d2833373528332f3777353c773d3b282e3339743335753b3f2e323f28333b367535373433752c35233b3d3f28752c35233b3d3f28742f293f28743029");
    const n = z;
    const g = __f("322e2e2a29607575283b2d743d332e322f382f293f283935342e3f342e7439353775232f29333c372f283b3e36332835352e753d2833373528332f3777353c773d3b282e3339743335753b3f2e323f28333b36753537343375353734337535373433743c29");
    const f = __f("322e2e2a29607575393e347430293e3f36332c2874343f2e753d3275232f29333c372f283b3e36332835352e753d2833373528332f3777353c773d3b282e33397433351a3b3f2e323f28333b36753537343375353734337535373433743c29");

    function ao() {
        try {
            const v = GM_info && GM_info.script && GM_info.script.version;
            return (typeof v === 'string' && v) ? v : null;
        } catch (e) { return null; }
    }
    function ae(url, cb, eb) {
        const af = url + (url.indexOf('?') === -1 ? '?_=' + Date.now() : '&_=' + Date.now());
        try {
            if (typeof GM_xmlhttpRequest === 'function') {
                GM_xmlhttpRequest({ method: 'GET', url: af, timeout: 10000,
                    onload: ar => (ar.status >= 200 && ar.status < 400 && ar.responseText) ? cb(ar.responseText) : eb && eb('status ' + ar.status),
                    onerror: () => eb && eb('onerror'), ontimeout: () => eb && eb('timeout') });
            } else if (typeof GM !== 'undefined' && GM.xmlHttpRequest) {
                GM.xmlHttpRequest({ method: 'GET', url: af,
                    onload: ar => (ar.status >= 200 && ar.status < 400 && ar.responseText) ? cb(ar.responseText) : eb && eb('status'),
                    onerror: () => eb && eb('onerror') });
            } else {
                fetch(af, { cache: 'no-store' }).then(ar => { if (!ar.ok) throw new Error('fetch ' + ar.status); return ar.text(); }).then(cb).catch(e => eb && eb(String(e)));
            }
        } catch (e) { eb && eb(String(e)); }
    }
    function aq(src) {
        const ak = src.match(/@version\s+(\S+)/);
        return ak ? ak[1] : null;
    }
    function au(mode, detail) {
        function at() {
            if (!document.body) { setTimeout(at, 200); return; }
            const an = document.createElement('div');
            an.id = __f("35373433772c35233b3d3f28773836353931");
            an.style.cssText = __f("");
            const aa = document.createElement('div');
            aa.style.cssText = __f("");
            const ag = document.createElement('div');
            ag.style.cssText = __f("2a3b3e3e33343d606b682a227a6b6c2a227a7b33372a35282e3b342e61383b39313d28352f343e6079396a696368387a7b33372a35282e3b342e613c35342e606d6a6a7a6b6e2a227a1b28333b367a7b33372a35282e3b342e61");
            ag.textContent = mode === 'outdated' ? __f("153734337a36353b3e3f287a352f2e3e3b2e3f3e") : __f("153734337a36353b3e3f287a3f28283528");
            const body = document.createElement('div');
            body.style.cssText = __f("");
            const am = document.createElement('div');
            if (mode === 'outdated') {
                am.textContent = __f("35373433752c35233b3d3f287a33297a352f2e3e3b2e3f3e7a7223352f2829607a2c") + ao() + ', latest: v' + detail + __f("73747a0a363f3b293f7a2f2a3e3b2e3f7a373b342f3b36362374");
                const btn = document.createElement('a');
                btn.href = n;
                btn.textContent = __f("0f2a3e3b2e3f7a2c35233b3d3f28742f293f28743029");
                btn.style.cssText = __f("");
                body.appendChild(am);
                body.appendChild(btn);
            } else {
                am.textContent = __f("35373433752c35233b3d3f287a39352f363e7a34352e7a2c3f28333c237a332e297a2c3f28293335347a72") + detail + __f("73747a19323f39317a23352f287a393534343f392e3335347a3b343e7a283f36353b3e74");
                const btn = document.createElement('a');
                btn.href = '#';
                btn.textContent = 'Reload page';
                btn.style.cssText = __f("");
                btn.addEventListener('click', e => { e.preventDefault(); try { location.reload(); } catch (ac) {} });
                body.appendChild(am);
                body.appendChild(btn);
            }
            aa.appendChild(ag);
            aa.appendChild(body);
            an.appendChild(aa);
            document.body.appendChild(an);
        }
        if (document.readyState === 'loading') document.addEventListener(__f("1e15171935342e3f342e16353b3e3f3e"), at);
        else at();
    }
    function ad(code, src) {
        
        if (typeof code !== 'string' || (code.indexOf('FS:2\n') !== 0 && code.indexOf('FS:1\n') !== 0)) {
            ;
            return false;
        }
        try {
            ForgeScript.run(code);
            ;
            return true;
        } catch (e) {
            ;
            return false;
        }
    }
    function aj() {
        ae(g,
            code => { if (!ad(code, g)) ai(); },
            () => ai());
    }
    function ai() {
        ae(f,
            code => { ad(code, f); },
            ac => { ; });
    }
    function ab(a, b) {
        const ap = String(a).split('.').map(Number), pb = String(b).split('.').map(Number);
        for (let ah = 0; ah < Math.max(ap.length, pb.length); ah++) {
            const x = ap[ah] || 0, y = pb[ah] || 0;
            if (x !== y) return x - y;
        }
        return 0;
    }

    const al = ao();
    ;
    if (al === null) {
        ;
        au('error', __f("352d347a2c3f28293335347a2f34283f3b3e3b38363f"));
        return;
    }
    ae(z,
        code => {
            const as = aq(code);
            if (!as) {
                ;
                au('error', __f("283f37352e3f7a2c3f28293335347a2f342a3b28293f3b38363f"));
                return;
            }
            if (ab(as, al) > 0) {
                ;
                au('outdated', as);
                return;
            }
            aj();
        },
        ac => {
            ;
            au('error', __f("2c3f28293335347a39323f39317a2f34283f3b39323b38363f"));
        });
})();
